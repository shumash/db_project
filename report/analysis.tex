...
Choosing a threshold $T$ requires weighing the quantitative benefits of compression with the qualitatively poorer image reconstructions. We ran a number of experiments, varying the threshold, and quantitative and qualitatively examining the results. In fig. \ref{fig:perfGraphs} we plot a few small experiments (with 200 images) for demonstrative purposes. 
TODO: Zoya will finish this section
...
\caption{Quantitative and qualitative results obtained by varying the patch similarity threshold, $T$, while extracting $400$ patches from each image. In the first row, $T=250$, the dictionary size is 50,822 patches, and the average compression ratio per image is $36.73\%$. In the second row, $T=25$, the dictionary size is $65,794$ patches, and the average compression ratio per image is $18.09\%$. In the third row, $T=2.5$, the dictionary size is $72,445$ patches, and the average compression ratio per image is $9.80\%$.}
