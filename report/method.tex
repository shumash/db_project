...
from all the other patches in \texttt{patch\_dict} are subsequently added to the dictionary (See Sec.~\ref{sec:sim}).
...
patch_hashes(patch_id int PRIMARY KEY REFERENCES patch_dict(id),
             hash int);
\end{verbatim}
