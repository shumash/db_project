...
\begin{algorithm}
    ...
    \label{alg:insert}
\begin{algorithmic}[1]
\State $Patches \leftarrow $ \texttt{Patchify}(I, patch\_size=$n$)
\For{$P_j$ in $Patches$}
...
We optimized our system for its performance. In the original algorithm designed, for each image I to insert, we have (m/n)^2 queries into the database to figure out whether there are patches that are similar to each of image Iís patches. Depends on whether there are similar patches in the database, we also send a query to insert a patch for tiles in the image. This means for a single image insertion, we have 2*(m/n)^2 queries into the database. This decreases the performance of our system. To solve this problem, we devised the following modified algorithm.

Algorithm for batch insert for a single image I
Patches <- CutIntoPatches(I,patch_sizes=n)
UniquePatchesWithinSingleImage <- FindUniquePatches(I)
SimilarPatchesInDB <- GetLikelySimilarPatches(UniquePatchesWithinSingleImage)
PatchesToStoreToDB = []
for P_i, P_j in SimilarPatches, UniquePatchesWithinSingleImage do:
     if S(P_i,P_j) > T
        PatchesToStoreToDB.Add(P_j)
BatchInsert(PatchesToStoreToDB)

This batch insert ensure that for a single image, we at most query the database twice, once for finding all the likely similar patches in the database, once to batch insert all the patches into the database. This improves the performance of our system by a scale of (m/n)^2 which is a significant improvement. 
The idea of this algorithm is that to do a local filtering among the patches of a single image before we query the database, such that the set of patches will only contains patches that are already greater than T distance away from each other.  Letís call the filtered set the unique set. Then we query the database to find the set of likely to be similar patches to each of the patches in the unique set we found locally for this image. Only if none of the likely-to-be similar patches in the database matches a patch in the unique set, we would later insert this patch. 
